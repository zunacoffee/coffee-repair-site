          ...
          <div className="grid gap-8 lg:grid-cols-[1.4fr_0.9fr]">
                  ...
                  {editing ? (
                    <>
                      ...
                      <div className="sm:col-span-2">
                        <label className="block text-sm font-medium text-[#0D1B2A]">Address</label>
                        <input value={contactForm.address} onChange={(e) => updateFormField('address', e.target.value)} className="mt-2 w-full rounded-2xl border border-[#E8ECF0] bg-[#F4F6F9] px-4 py-3 text-sm text-[#0D1B2A] focus:border-[#B87333] focus:outline-none focus:ring-2 focus:ring-[#B87333]/20" />
                      </div>
                    </>
                  ) : (
            ...
            <aside className="space-y-6">
              <section className="rounded-3xl border border-[#E8ECF0] bg-white p-8 shadow-sm">
                ...
                <p className="mt-2 text-sm text-[#7A8898]">Current plan status and renewal information.</p>
                <div className="mt-6 space-y-4 text-sm text-[#0D1B2A]">
                  {plan ? (
                    <>
                      <div>
                        <p className="text-sm font-semibold text-[#0D1B2A]">{plan.plan_name}</p>
                        <p className="mt-1 text-[#7A8898]">Status: {plan.status}</p>
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-[#0D1B2A]">Price</p>
                        <p className="mt-1 text-[#7A8898]">${plan.price.toFixed(2)} / month</p>
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-[#0D1B2A]">Renewal</p>
                        <p className="mt-1 text-[#7A8898]">{plan.renewal_date ? new Date(plan.renewal_date).toLocaleDateString() : 'Not scheduled'}</p>
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-[#7A8898]">No maintenance plan is currently assigned.</p>
                  )}
                </div>
              </section>
            </aside>
